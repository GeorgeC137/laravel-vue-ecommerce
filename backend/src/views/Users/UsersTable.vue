<template>
    <div class="bg-white shadow p-4 rounded-lg animate-fade-in-down">
        <div class="flex justify-between border-b-2 pb-3">
            <div class="flex items-center">
                ...
                <select
                    ...
                    @change="getUsers()" v-model="perPage">
            ...
            <div>
                <input v-model="search" @change="getUsers()"
        ...
        <table class="table-auto w-full">
            <thead>
                <tr>
                    <TableHeaderCell @click="sortUsers" class="border-b-2 p-2 text-left" field="id" :sort-field="sortField"
                        :sort-direction="sortDirection">ID</TableHeaderCell>
                    <TableHeaderCell @click="sortUsers" class="border-b-2 p-2 text-left" field="name"
                        :sort-field="sortField" :sort-direction="sortDirection">Name</TableHeaderCell>
                    <TableHeaderCell @click="sortUsers" class="border-b-2 p-2 text-left" field="email"
                        :sort-field="sortField" :sort-direction="sortDirection">Email</TableHeaderCell>
                    <TableHeaderCell @click="sortUsers" class="border-b-2 p-2 text-left" field="updated_at"
            ...
            <tbody v-if="users.loading || !users.data.length">
                <tr>
                    <td colspan="5">
                        ...
                        <p v-else class="text-gray-700 py-8 text-center">There are no users</p>
                    </td>
                </tr>
            </tbody>

            <tbody v-else>
                <tr v-for="(user, ind) in users.data">
        ...
        <div v-if="!users.loading && users.data.length" class="flex justify-between items-center mt-5">
            ...
            <nav v-if="users.total > users.limit"
                ...
                <a href="#" @click="getCurrentPage($event, link)" v-html="link.label" :disabled="!link.url"
                    v-for="(link, ind) in users.links" :key="ind"
                    class="relative inline-flex items-center px-4 py-2 border text-sm font-medium whitespace-nowrap" :class="[
                        ...
                        ind === 0 ? 'rounded-l-md' : '',
                        ind === users.links.length - 1 ? 'rounded-r-md' : '',
